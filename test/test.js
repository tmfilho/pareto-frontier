...
describe('pareto-frontier', function tests() {
	...
	it('should calculate the pareto frontier for topRight', function test() {
		expect(paretoFrontier.getParetoFrontier(reflectGraph(graph, true, false), {
			optimize: 'topRight'
		})).to.deep.equal(reflectGraph(expected, true, false));
	});

	it('should calculate the pareto frontier for bottomRight', function test() {
		expect(paretoFrontier.getParetoFrontier(reflectGraph(graph, true, true), {
			optimize: 'bottomRight'
		})).to.deep.equal(reflectGraph(expected, true, true));
	});

	it('should calculate the pareto frontier for bottomLeft', function test() {
		expect(paretoFrontier.getParetoFrontier(reflectGraph(graph, false, true), {
			optimize: 'bottomLeft'
		})).to.deep.equal(reflectGraph(expected, false, true));
	});

	it('should default to the topRight pareto frontier', function test() {
		expect(paretoFrontier.getParetoFrontier(graph)).to.deep.equal(paretoFrontier.getParetoFrontier(
			graph, {
				optimize: 'topRight'
			}));
	});
